...
| `rns.pill.lora.*` | The top status line's LoRa pill (yellow `ffd400`, order 4), written through rnsd: `L` and the number of other nodes heard, summed over every slot — a board with two radios still has one LoRa neighbourhood. Published while any slot is enabled, at 0 as readily as at 3, and taken down by the switch rather than by the beat (turning the last radio off parks the interface task). See [rns/README](../rns/README.md#status-line-pills). |
