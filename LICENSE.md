# License

This repository, **reticulous-lora** (SX1262 LoRa transport for reticulous;
RadioLib + custom ESP-IDF HAL, RNode on-air framing), is released under the
**Apache License, Version 2.0**.

Full license text: <https://www.apache.org/licenses/LICENSE-2.0>

Copyright (c) 2026 by reticulous project contributors.

## Third-party software

### Vendored in this repository

None. (The RadioLib ESP-IDF HAL, `esp_idf_hal.{cpp,h}`, is original spangap
glue and ships under Apache-2.0 alongside the rest of this repo.)

### Build-time dependencies

Declared in `esp-idf/idf_component.yml` and `browser/package.json`:

| Component / package | Source | License |
|---|---|---|
| ESP-IDF (platform) | espressif/esp-idf | Apache-2.0 |
| `jgromes/radiolib` v7 | components.espressif.com / GitHub | **MIT** |
| Browser peer deps (Vue, Quasar, Pinia, vue-router) | npm | MIT |
